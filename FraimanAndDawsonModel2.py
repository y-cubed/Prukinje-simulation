...
tpnt = numpy.arange(0.0, 0.301, 0.001)
res = numpy.zeros([NITER, 301, 4])
res_std = numpy.zeros([301, 4])
res_std1 = numpy.zeros([301, 4])
res_std2 = numpy.zeros([301, 4])
...
for i in range(0, NITER):
        ...
        sim.setPatchCount('memb', 'A00', 10)
        sim.setCompConc('cyt', 'IP3', 10e-6)
        sim.setCompClamped('cyt', 'IP3', True)
        sim.setCompConc('cyt', 'Ca', ca_concs[14])
        #sim.setCompClamped('cyt', 'Ca', True)
...
#pylab.plot(tpnt, res_mean[:, 4], linewidth = 2.0, label = 'Oa+Ob+Oc')
pylab.plot(tpnt, res_mean[:, 3], linewidth = 2.0, label = 'Ca in cytosole')
