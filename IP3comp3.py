...
Pa_bind_IP3_f = smodel.SReac('Pa_bind_IP3_f', surfsys, olhs = [IP3], slhs = [Pa], srhs = [Oa])
Pa_bind_IP3_b = smodel.SReac('Pa_bind_IP3_b', surfsys, slhs = [Oa], orhs = [IP3], srhs = [Pa])
Pa_bind_IP3_f.setKcst(1540e6)
Pa_bind_IP3_b.setKcst(18)
...
ca_concs = numpy.array([0.001e-6, 0.003e-6, 0.005e-6, 0.007e-6, 0.009e-6, 0.011e-6, 0.013e-6, 0.016e-6, 0.02e-6, 0.03e-6, 0.04e-6, 0.05e-6, 0.06e-6, 0.07e-6, 0.10e-6, 0.13e-6, 0.20e-6, 0.27e-6, 0.28e-6, 0.30e-6, 0.33e-6, 0.4e-6, 0.50e-6, 0.6e-6, 0.7e-6, 0.8e-6, 1.00e-6, 1.50e-6, 3.00e-6, 5e-6, 10.00e-6, 20e-6, 30.00e-6, 100.00e-6]) # mol/l
...
cn_values = numpy.array([0.0000002, 0.0000005, 0.0000008, 0.000001, 0.0000015, 0.000002, 0.00001, 0.00002, 0.00003, 0.00004, 0.00005, 0.00007, 0.0001, 0.00015, 0.00018])
...
for ip in cn_values: 
        ...
        for ca in ca_concs:   #ca_concs for
                ...
                for i in range(0, NITER):
                        ...
                        for t in range(0, 3001):
                            ...
                            """
                            print t
                            print 'A:', sim.getPatchCount('memb', 'A00'), sim.getPatchCount('memb', 'A01'), sim.getPatchCount('memb', 'A10'), sim.getPatchCount('memb', 'A11')
                ...
                """
                pylab.plot(tpnt, res_mean[:, 0], linewidth = 2.0, label = 'Oa')
                pylab.plot(tpnt, res_mean[:, 1], linewidth = 2.0, label = 'Ob')
                pylab.plot(tpnt, res_mean[:, 2], linewidth = 2.0, label = 'Oc')
                if deterministic == False:
                    pylab.plot(tpnt, res_std1, color = 'gray', linewidth = 1.0, label = 'Oa std')
        ...
        pylab.plot(ca_concs*1e6, prob, label = str(ip))
        ...
        pyplot.xscale('log')
        pylab.title('Open probability of IP3 receptors')

pyplot.legend()
pyplot.savefig("IP3comp3_3.png")
